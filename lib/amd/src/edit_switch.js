...
export const init = editingSwitchId => {
    ...
    editSwitch.addEventListener('change', () => {
        if (window.location.href.indexOf('course/view.php') !== -1) {
            // Scroll to subject in case this is a course, expanded / collapsed course section headers y position differs.
            editSwitch.dataset.pageurl = scrollManager.saveScrollSubject(editSwitch.dataset.pageurl, 'activity', 'course-section',
                false);
        } else {
            editSwitch.dataset.pageurl = scrollManager.setPageUrlParam(editSwitch.dataset.pageurl, 'mdlscrollto',
                scrollManager.getScrollPos(), false);
        }
